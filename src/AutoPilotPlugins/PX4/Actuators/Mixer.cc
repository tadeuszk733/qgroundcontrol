        ...
        QObject(parent), _label(label), _actuatorFunction(actuatorFunction), _paramIndex(paramIndex), _actuatorTypeIndex(actuatorTypeIndex)
{
    for (int i = 0; i < channelConfigs.count(); ++i) {
        ...
        QString sparamActuatorTypeIndex = QString::number(actuatorTypeIndex + channelConfig->indexOffset());
        QString param = channelConfig->parameter();
        if (channelConfig->isActuatorTypeConfig()) {
            param.replace("${i}", sparamActuatorTypeIndex);
        } else {
...
void MixerController::reset(const ActuatorTypes& actuatorTypes, const MixerOptions& mixerOptions,
    ...
    if (_selectedMixer != -1) {
        ...
        for (const auto &actuatorGroup : actuatorGroups) {
            ...
            for (const auto& parameter : actuatorGroup.parameters) {
                currentMixerGroup->addConfigParam(new ConfigParameter(currentMixerGroup, getFact(parameter.param.name),
                        parameter.param.label, parameter.param.advanced));
            }
...
QString MixerController::getSpecificLabelForFunction(int function) const
{
...
QSet<int> MixerController::requiredFunctions() const
{
    QSet<int> ret;
...
QString MixerController::configuredType() const
{
    if (_selectedMixer == -1) {
        return "";
    }
...
Fact* MixerController::getFact(const QString& paramName)
{
...
void MixerController::subscribeFact(Fact* fact)
{
    if (fact && !_subscribedFacts.contains(fact)) {
        connect(fact, &Fact::rawValueChanged, this, &MixerController::paramChanged);
        _subscribedFacts.insert(fact);
    }
}

void MixerController::unsubscribeFacts()
{
    for (Fact* fact : _subscribedFacts) {
        disconnect(fact, &Fact::rawValueChanged, this, &MixerController::paramChanged);
    }
