...
ActuatorTestController::ActuatorTestController(Vehicle* vehicle)
    : _vehicle(vehicle)
{
    _watchdogTimer.setInterval(100);
    _watchdogTimer.setSingleShot(false);
    connect(&_watchdogTimer, &QTimer::timeout, this, &ActuatorTestController::watchdogTimeout);
    _watchdogTimer.start();
}

ActuatorTestController::~ActuatorTestController()
{
    _actuators->clearAndDeleteContents();
    delete _allMotorsActuator;
}

void ActuatorTestController::updateFunctions(const QList<Actuator*> &actuators)
{
    _actuators->clearAndDeleteContents();
    delete _allMotorsActuator;
...
void ActuatorTestController::resetStates()
{
    _states.clear();
    _currentState = -1;
...
void ActuatorTestController::watchdogTimeout()
{
...
void ActuatorTestController::setChannelTo(int index, float value)
{
...
void ActuatorTestController::stopControl(int index)
{
...
void ActuatorTestController::setActive(bool active)
{
...
void ActuatorTestController::ackHandlerEntry(void* resultHandlerData, int compId, MAV_RESULT commandResult,
        Vehicle::MavCmdResultFailureCode_t failureCode)
{
    ActuatorTestController* controller = (ActuatorTestController*)resultHandlerData;
    controller->ackHandler(commandResult, failureCode);
}

void ActuatorTestController::ackHandler(MAV_RESULT commandResult, Vehicle::MavCmdResultFailureCode_t failureCode)
{
...
void ActuatorTestController::sendNext()
{
...
void ActuatorTestController::sendMavlinkRequest(int function, float value, float timeout)
{
    ...
    _vehicle->sendMavCommandWithHandler(
            ...
            MAV_CMD_DO_ACTUATOR_TEST,         // the mavlink command
            value,                            // value
            timeout,                          // timeout
            0,                                // unused parameter
            0,                                // unused parameter
