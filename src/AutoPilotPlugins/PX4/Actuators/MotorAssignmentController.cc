...
MotorAssignmentController::MotorAssignmentController(QObject* parent, Vehicle* vehicle, QmlObjectListModel* actuators)
        : QObject(parent), _vehicle(vehicle), _actuators(actuators)
{
    _spinTimer.setInterval(1000);
    _spinTimer.setSingleShot(true);
    connect(&_spinTimer, &QTimer::timeout, this, &MotorAssignmentController::spinTimeout);
}

bool MotorAssignmentController::initAssignment(int selectedActuatorIdx, int firstMotorsFunction, int numMotors)
{
...
void MotorAssignmentController::start()
{
...
void MotorAssignmentController::selectMotor(int motorIndex)
{
...
void MotorAssignmentController::spinCurrentMotor()
{
...
void MotorAssignmentController::abort()
{
    _spinTimer.stop();
    _state = State::Idle;
    emit activeChanged();
...
void MotorAssignmentController::spinTimeout()
{
    spinCurrentMotor();
}

void MotorAssignmentController::ackHandlerEntry(void* resultHandlerData, int compId, MAV_RESULT commandResult,
        Vehicle::MavCmdResultFailureCode_t failureCode)
{
    MotorAssignmentController* controller = (MotorAssignmentController*)resultHandlerData;
    controller->ackHandler(commandResult, failureCode);
}

void MotorAssignmentController::ackHandler(MAV_RESULT commandResult, Vehicle::MavCmdResultFailureCode_t failureCode)
{
...
void MotorAssignmentController::sendMavlinkRequest(int function, float value)
{
