...
ActuatorOutput* ActuatorsController::selectedActuatorOutput() const
{
...
void ActuatorsController::updateAirframeImage()
{
    GeometryImage::VehicleGeometryImageProvider* provider = GeometryImage::VehicleGeometryImageProvider::instance();
    ...
    actuators.clear();
    for (int mixerGroupIdx = 0; mixerGroupIdx < _mixerController.groups()->count(); ++mixerGroupIdx) {
        Mixer::MixerConfigGroup* mixerGroup = _mixerController.groups()->value<Mixer::MixerConfigGroup*>(mixerGroupIdx);
        for (int mixerChannelIdx = 0; mixerChannelIdx < mixerGroup->channels()->count(); ++mixerChannelIdx) {
            ...
            if (mixerChannel->getGeometry(_mixerController.actuatorTypes(), mixerGroup->group(), geometry)) {
...
bool ActuatorsController::isMultirotor() const
{
    return _mixerController.configuredType() == "multirotor";
}

void ActuatorsController::load(const QString &json_file)
{
    QFile file;
    ...
    parseJson(QJsonDocument::fromJson(json_data.toUtf8()));
...
void ActuatorsController::parametersChanged()
{
    qCDebug(ActuatorsConfigLog) << "Param update";

    _mixerController.update();
    ...
    for (int groupIdx = 0; groupIdx < _actuatorOutputs->count(); groupIdx++) {
        ...
        for (const auto& groupFunction : groupFunctions) {
            ...
            const auto iter = _mixerController.functions().find(function);
            if (iter != _mixerController.functions().end() && iter->note != "") {
    ...
    const Mixer::ActuatorTypes &actuatorTypes = _mixerController.actuatorTypes();
    for (int function : allFunctions) {
        ...
        QString label = _mixerController.getSpecificLabelForFunction(function);

        // check if we should exclude the function from testing
        bool excludeFromActuatorTesting = false;
        const auto iter = _mixerController.functions().find(function);
        if (iter != _mixerController.functions().end()) {
        ...
        if (!excludeFromActuatorTesting) {
            ...
            for (const auto& actuatorTypeName : actuatorTypes.keys()) {
                ...
                if (function >= actuatorType.functionMin && function <= actuatorType.functionMax) {
                    ...
                    actuators.append(
                            new ActuatorTesting::Actuator(&_actuatorTestController, label, actuatorType.values.min, actuatorType.values.max,
    ...
    _actuatorTestController.updateFunctions(actuators);

    // check if there are required functions, but not set on any output
    QSet<int> requiredFunctions = _mixerController.requiredFunctions();
...
void ActuatorsController::parseJson(const QJsonDocument &json)
{
    ...
    for (const auto &outputJson : outputs) {
        ...
        Condition groupVisibilityCondition(output["show-groups-if"].toString(""), _vehicle->parameterManager());
        ...
        for (const auto& subgroupJson : subgroups) {
            ...
            for (const auto& channelParametersJson : channelParameters) {
                ...
                } else if (functionStr == "minimum") {
                    function = ChannelConfig::Function::Minimum;
                } else if (functionStr == "maximum") {
    ...
    // parse functions
    QMap<int, Mixer::MixerController::OutputFunction> outputFunctions;
    ...
    for (const auto& functionKey : functions.keys()) {
        ...
        if (ok) {
            ...
            if (label != "") {
                ...
                outputFunctions[key] = Mixer::MixerController::OutputFunction{label, conditionObj, note, exclude};
            }
        }
    }
    ...
    _mixerController.reset(actuatorTypes, mixerOptions, outputFunctions);
    _hasMetadata = true;
}

Fact* ActuatorsController::getFact(const QString& paramName)
{
    if (!_vehicle->parameterManager()->parameterExists(FactSystem::defaultComponentId, paramName)) {
        qCDebug(ActuatorsConfigLog) << "MixerController: Param does not exist:" << paramName;
        return nullptr;
    }
...
void ActuatorsController::subscribeFact(Fact* fact)
{
    if (fact && !_subscribedFacts.contains(fact)) {
        connect(fact, &Fact::rawValueChanged, this, &ActuatorsController::parametersChanged);
